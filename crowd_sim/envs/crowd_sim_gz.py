...
class CrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        logging.info("Using {} kinematics".format(config.action_space.kinematics))
        ...
        if self.dynamics == 'ballbot':
            logging.info("Using BallBot Dynamics")
    ...
    def generate_lab_simulation(self, i, stored_points):
        human = Human(self.config, 'humans')
        # print(stored_points)
        px, py = stored_points['sphere{}'.format(i+1)]['start'][0]
        gx, gy = stored_points['sphere{}'.format(i+1)]['goal'][0]
        human.set(px, py, gx, gy, 0, 0, 0)
        return human
    ...
    def render(self, mode='human'):
        ...
        # add lean angle notation
        if self.robot.dynamics == 'ballbot' and len(self.robot._dynamics.lean_angles) > 0:
            angle = plt.text(-2, 4,
                                'Leaning Angle: {:.4f}'.format(self.robot._dynamics.lean_angles[-1]), fontsize=16)
            ax.add_artist(angle)
